...
public class GraphHelper {
	...
	/**
	 * delaunay triangulation k'th level
	 */
	public static HashSet<Integer>[] delaunayKNN(TTPInstance ttp, int k) {
		...
		for (int i = 0; i < nbCities; i++) { // Deb.echo(i+"-"+tri[i]);
			for (int j = 1; j <= k - 1; j++) {
				HashSet<Integer> currentSet = new HashSet(tri2[i]);
				for (int x : currentSet) { // Deb.echo(">"+x+":"+tri[x]);
					tri2[i].addAll(tri[x]);
				}
				// Deb.echo(i + "-" + tri2[i]);
			}
	...
	public static void main(String[] args) {
		...
		// final TTP1Instance ttp = new
		// TTP1Instance("berlin52-ttp/berlin52_n51_bounded-strongly-corr_01.ttp");
