...
/**
 * local search algorithms based on TSP's N1 and KP's Bit-flip
 * 
 * @author kyu
 *
 */
public class JNB extends LocalSearch {
	...
	public TTPSolution search() {
		...
		long wc, // current weight
				newWA, oldWA; // saved WA at swap index
		...
		do {
			...
			for (int i = 1; i < nbCities - 1; i++) {
				...
				// compute objective
				deltaT = -D[c1][c2] / v1 - D[c2][c3] / v2 - D[c3][c4] / v3 + D[c1][c3] / v1 + D[c3][c2] / v2i
						+ D[c2][c4] / v3;
				...
				// }
				if (i - 2 > -1)
					tacc[i - 2] = sol.timeAcc[i - 2]; // @todo sufficient ?
				...
				for (int k = 0; k < nbItems; k++) {
					...
					/*
					 * velocity-TSP TSP constrained with knapsack weight
					 */
