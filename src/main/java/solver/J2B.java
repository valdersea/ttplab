...
/**
 * local search algorithms based on TSP's 2-opt and KP's Bit-flip
 * 
 * @author kyu
 *
 */
				...
				for (j = i + 1; j < nbCities; j++) {
					...
					for (k = 0; k < nbItems; k++) {
						...
						for (int r = newBF; r < nbCities; r++) {
							...
							int c2 = TwoOptHelper.get2optValue((r + 1) % nbCities, tour, i, j) - 1; // todo: avoid using
																									// the %
																									// operator...
						...
						// Deb.echo("("+i+","+j+","+k+") iBF:"+refBF+" || "+"ft: "+ ft + " | G: " + G +
						// " | fp: "+fp);
