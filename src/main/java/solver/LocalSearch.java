...
public abstract class LocalSearch extends SearchHeuristic {
	...
	public LocalSearch(TTP1Instance ttp) {
		...
		if (!System.getProperty("os.name").contains("Windows")) {
			...
			if (debug)
				Deb.echo("Delaunay triangulation: OK");
		}
	}
	 ...
	 * KP pre-processing
	 *
	 * base on the item insertion heuristic in MATLS described in "improving
	 * efficiency of heuristics..."
	 *
	 * uses only delta_1 and delta_2 approximations in order to limit the number of
	 * inserted items
	 */
	...
	public TTPSolution insertAndEliminate(TTPSolution sol) {
		...
		for (itr = 0; itr < nbItems; itr++) {
			...
			if (ttp.profitOf(k) > R * t2) {
				...
				b2 = maxSpeed - C * wCurr;
				t3 = (1 / a) * Math.log(((a * L[0] + b1) * (a * (L[0] - L[origBF]) + b2))
						/ ((a * (L[0] - L[origBF]) + b1) * (a * L[0] + b2)));
				if (ttp.profitOf(k) > R * t3) {
					v3++;
					pickingPlan[k] = A[k];
					wCurr += ttp.weightOf(k);
					insertedItems[nbInserts++] = k;
		...
		if (nbItems > 100000 || nbCities > 50000)
			return sol;
	 ...
	 * 2-opt search
	 *
	 * deal with the TSKP sub-problem 2-opt heuristic with Delaunay candidate
	 * generator
	 */
	public TTPSolution fast2opt(TTPSolution sol) {
		...
		do {
			...
			// debug msg
			if (this.debug) {
				Deb.echo(">> TSKP " + nbIter + ": ob=" + String.format("%.0f", sol.ob) + " | ft="
						+ String.format("%.0f", sol.ft));
			}
	 ...
	 * 2-opt search
	 *
	 * deal with the TSKP sub-problem 2-opt heuristic without neighborhood reduction
	 * techniques
	 */
	...
	public TTPSolution lsBitFlip(TTPSolution sol) {
		...
		do {
			...
			if (improved) {
				...
				// debug msg
				if (this.debug) {
					Deb.echo(">> KRP: " + nbIter + " | ob=" + String.format("%.2f", sol.ob) + " | ft="
							+ String.format("%.2f", sol.ft));
				}
			}
