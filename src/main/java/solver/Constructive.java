...
public class Constructive extends TTPHeuristic {
	...
	/**
	 * simple greedy algorithm based on distances between nodes
	 * 
	 * @return the found tour
	 */
	...
	/**
	 * use Lin-Kernighan TSP tour uses hardcoded tours
	 */
	...
	public int[] chainedLKTour() {
		...
		try {
			...
			String[] cmd = { "./bins/linkern/linkern.sh", fileName, "" + (1 + Math.random()),
					"" + RandGen.randInt(1, nbCities) };
	...
	public int[] qburuvkaTour() {
		...
		try {
			// execute linkern program
			String[] cmd = { "./bins/kdtree/kdtree", "-j", "-o", "./bins/kdtree/out.tour", fileName };
			Runtime runtime = Runtime.getRuntime();
			Process proc = runtime.exec(cmd);
	...
	public int[] greedyPickingPlan(int h) {
		...
		int m = ttp.getNbCities(), n = ttp.getNbItems();
	...
	/**
	 * basic heuristic no 1 based on the constructive heuristic SH
	 * 
	 * sub-TSP: greedy algorithm sub-KP: created for the fixed TSP tour so it
	 * maximizes TTP value
	 * 
	...
	public TTPSolution SH2() {
		...
		int m = ttp.getNbCities(), n = ttp.getNbItems();
		TTPSolution s = new TTPSolution(m, n);
		int[] x = s.getTour(), z = s.getPickingPlan();
		...
		/*
		 * the picking plan generated so that the TTP objective value is maximized
		 */
		ttp.objective(s);
		...
		for (int k = 0; k < n; k++) {
			...
			int iw = ttp.weightOf(k), ip = ttp.profitOf(k);
